...
# --- Command Runner ---
def run_command(cmd, cwd=None, check=True): # Added check parameter
    """Runs a command, logs it, and handles potential errors."""
    # Ensure all parts are strings for logging and execution
    cmd_str_list = [str(c) for c in cmd]
    logger.info(f"[RUN] CWD: {cwd or '.'} | CMD: {' '.join(cmd_str_list)}")
    try:
        # Use check=True by default, but allow overriding
        result = subprocess.run(cmd_str_list, check=check, cwd=cwd, capture_output=True, text=True, encoding='utf-8', errors='replace')
        ...
        logger.error(f"[FAIL] Command not found: {cmd_str_list[0]} - {e}")
        sys.exit(1) # Exit if command not found
    except subprocess.CalledProcessError as e:
        logger.error(f"[FAIL] Command failed with exit code {e.returncode}: {' '.join(cmd_str_list)}")
        ...
        logger.error(f"[FAIL] Unexpected error running command: {e_generic}", exc_info=True)
